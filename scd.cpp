...
bool check_sequence(const std::vector<int>& v){
    ...
    for (int i = 1 ; i<=v.size() ; i++){
        if(mfind(i,v) == -1){
            return everynum = false ;
        }
    }
    return everynum = true ;
}
...
void get_row(int r, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int rowlen = sqrt(in.size()) ; 
    for (int i = 0 ; i < rowlen ; i++){
        out.push_back(in[twod_to_oned(r,i,rowlen)]);
    }
}
...
void get_col(int c, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int collen = sqrt(in.size());
    for (int i = 0 ; i < collen ; i++){
        out.push_back(in[twod_to_oned(i,c,collen)]) ;
    }
}
...
void get_subsq(int subs, const std::vector<int>& in, std::vector<int>& out){
    // write your code here
    int row_len = sqrt(in.size()) ;
    int sub_len = sqrt(row_len) ;

    int sub_row =  (subs/sub_len) * sub_len ; 
    int sub_col = (subs%sub_len)*sub_len ;

    for (int i = sub_row ; i < sub_row + sub_len ; i++){
        for (int j = sub_col ; j < sub_col + sub_len ; j++){
            out.push_back(in[twod_to_oned(i,j,row_len)]) ;
        }
    }
}
